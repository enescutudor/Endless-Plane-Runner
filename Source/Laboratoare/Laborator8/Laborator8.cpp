...
void Laborator8::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
	...
	int mode_location = glGetUniformLocation(shader->program, "mode_location");
	glUniform1i(mode_location, isSpotlightOn?1:0);
...
void Laborator8::OnInputUpdate(float deltaTime, int mods)
{
	...
	if (!window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
	{
		...
		if (window->KeyHold(GLFW_KEY_LEFT)) lightDirection -= forward * deltaTime * speed;
		if (window->KeyHold(GLFW_KEY_4)) lightDirection -= right * deltaTime * speed;
		if (window->KeyHold(GLFW_KEY_2)) lightDirection += forward * deltaTime * speed;
		if (window->KeyHold(GLFW_KEY_6)) lightDirection += right * deltaTime * speed;
		if (window->KeyHold(GLFW_KEY_7)) lightDirection += up * deltaTime * speed;
		if (window->KeyHold(GLFW_KEY_9)) lightDirection -= up * deltaTime * speed;
	}
}
